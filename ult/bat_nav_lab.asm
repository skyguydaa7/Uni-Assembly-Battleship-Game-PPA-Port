...
fSLEEP proc ;aprox 1 segundo
  push AX
  push BX
  push CX

  mov bx, 1000d
  mov ax, 100d ; Move o número de ms em um segundo para AX 
  mul bx       ; Faz AX = número de ms a esperar 
  mov cx, dx   ; Prepara para o delay - põe número de ms 
  mov dx, ax   ; onde necessário 
  mov ah, 86h  ; Cria o delay 
  int 15h      ;chama interrup.
...
POS_ACERTADA proc
  ...
  xor BX, BX
  mov  BH, 00h
  mov  AL, 08h    ; Verifica se a posição está pintada
  mov  Ah, 08h    ; Verifica se a posição está pintada
  int  10h
